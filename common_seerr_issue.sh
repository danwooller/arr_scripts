resolve_seerr_issue() {
    local folder_path="${1%/}"
    log "🔍 DEBUG: Starting resolve for: $folder_path"
    ...
    if [[ "$folder_path" == *"/TV/"* ]]; then
        ...
        log "🔍 DEBUG: Detected TV. Show Folder identified as: $show_folder"
        
        # Capture the raw Sonarr response to see why the match fails
        local sonarr_json=$(curl -s -H "X-Api-Key: $SONARR_API_KEY" "$SONARR_API_BASE/series")
        lookup_id=$(echo "$sonarr_json" | jq -r --arg path "$show_folder" '.[] | select(.path == $path or .path == ($path + "/")) | .tvdbId')
        
        log "🔍 DEBUG: Sonarr Search Path: '$show_folder'"
        log "🔍 DEBUG: Sonarr ID Found: '$lookup_id'"
        
        if [[ -z "$lookup_id" || "$lookup_id" == "null" ]]; then
            log "❌ DEBUG: Match failed. List of valid Sonarr paths:"
            echo "$sonarr_json" | jq -r '.[] | "   - " + .path' | head -n 5
        fi
    else
        # Movie logic...
