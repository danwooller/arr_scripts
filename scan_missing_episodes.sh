...
report_missing_seerr() {
    ...
    # Clean name: remove year and clean separators for search
    ...
    if [ -z "$media_id" ] || [ "$media_id" == "null" ]; then
        log "WARN: Could not link $series_name to Seerr to report missing eps."
        return 1
    fi

    # 2. Create Issue (Type 2: Technical/Other)
    local json_payload=$(jq -n \
        --arg mt "2" \
        --arg msg "Missing Episode(s) Gap: $missing_episodes. Check Sonarr for indexing issues." \
        --arg id "$media_id" \
...
log "Scanning $TARGET_DIR for missing episode gaps..."

# 1. Find all directories (Series)
find "$TARGET_DIR" -maxdepth 1 -mindepth 1 -type d | while read -r series_path; do
    series_name=$(basename "$series_path")
    
    # 2. Collect all episode patterns, excluding Specials/Season 00
    # This captures the 1x02 format
    mapfile -t ep_list < <(find "$series_path" -type f -not -path "*Specials*" -not -path "*Season 00*" \
        -name "*[0-9]x[0-9]*" | grep -oE "[0-9]+x[0-9]+" | sort -V | uniq)

    if [ ${#ep_list[@]} -lt 2 ]; then
        continue # Need at least 2 episodes to find a "gap"
    fi
    ...
    for ep in "${ep_list[@]}"; do
        ...
        curr_e=$(echo "$ep" | cut -d'x' -f2 | sed 's/^0//') # Remove leading zero for math

        if [ "$curr_s" -eq "$prev_s" ]; then
            expected=$((prev_e + 1))
            if [ "$curr_e" -gt "$expected" ]; then
                # Gap detected
                for ((i=expected; i<curr_e; i++)); do
